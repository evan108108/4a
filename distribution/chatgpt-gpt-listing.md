...
**Build target:** v0.5 (2026-04-28). The GPT wraps the 4A REST surface (`https://4a4.ai/surfaces/chatgpt-action.json`) and the OAuth 2.1 publishing flow. The surface now spans **15 reserved kinds** across two blocks:

- **Public block (v0 + Phase 3 v0):** Observation (30500), Claim (30501), Entity (30502), Relation (30503), Commons (30504), Score (30506), Comment (30507).
- **v0.5 audience block:** EncryptedObservation (30510), EncryptedClaim (30511), EncryptedEntity (30512), EncryptedRelation (30513), EncryptedCommons (30514), Audience (30520), KeyGrant (30521), AudienceClaim (30522).
- **Reserved for Sonata Studio:** 30530–30539 (federated multi-Sonata workspaces; the reference application proving the substrate is real).

Read actions are public; public-mode writes (`publishObservation`, `publishClaim`, `publishEntity`, `publishRelation`, `publishScore`, `publishComment`, `attest`) and the v0.5 audience-lifecycle actions (`createAudience`, `inviteToAudience`, `grantAudienceKey`, `claimAudience`, `rotateAudience`, `processAudienceClaims`, `listPendingAudienceClaims`, `listMyAudiences`, `publishToAudience`, `audienceInbox`) require Google sign-in, which derives the user's Nostr key from a non-extractable HMAC key in AWS KMS — no per-user keys are stored.

Phase 3 v0 added the credibility primitives: **`publishScore` is paired-publish** — it signs a `kind:30506` Score and an explanatory `kind:30507` Comment together, with cross-references in both directions, so aggregators can enforce the paired-rationale convention. **`publishComment`** is the standalone variant that lets anyone reply to or rebut any 4A object, recursively. Aggregators on this format treat unjustified scores as weight-zero — the rationale is required, not optional.

v0.5 added **private audiences** with NIP-44 v2 group encryption and NIP-17 gift-wrapping. Founders create audiences via `createAudience`, mint `4a://invite/...` URLs via `inviteToAudience`, admit pending claims via `processAudienceClaims` (or preview them first via `listPendingAudienceClaims`), and rotate the epoch when memberships change. Members publish encrypted variants via `publishToAudience` (one polymorphic tool covers all of 30510–30514) and decrypt their audience inbox via `audienceInbox`. Outside the gift-wrap envelope, relays see only `kind:1059` with a single `p` tag and opaque NIP-44 ciphertext — the audience slug, epoch, payload kind, member roster, and publisher pubkey are not on the wire. Migration target: [NIP-104 / MLS-on-Nostr](https://github.com/nostr-protocol/nips/blob/master/104.md) once stable.
...
4A is a vendor-neutral knowledge commons on Nostr. The public commons is signed and public-by-design — observations about software projects, claims about organizations, descriptions of entities, scores about other publishers' work, and recursive comments that reply to, rebut, or justify any of the above. **As of v0.5 (shipped 2026-04-28), the same surface also drives private 4A audiences** — named groups with NIP-44 group encryption, per-epoch key-grants, encrypted-variant kinds 30510–30514, and an off-band `4a://invite/...` claim flow. This GPT lets you read the public commons, publish to it under your derived Nostr identity, and, if you sign in, drive the full audience lifecycle without leaving the chat.
...
**Publishing is custodial and OAuth-gated.** When you first ask the GPT to record an observation, claim, entity, relation, score, comment, or to create / publish to an audience, ChatGPT will prompt you to sign in with Google. After consent, the gateway derives a Nostr keypair from your Google identity using a non-extractable HMAC key in AWS KMS — no private key is stored anywhere, by us or by anyone. The same Google account always derives the same 4A pubkey. Your Google identity *is* the recovery mechanism.

**Scoring is paired with rationale.** Phase 3 v0 added two event kinds: `kind:30506` Score (a numeric weight on any addressable 4A object) and `kind:30507` Comment (a free-text reply, addressable so anyone else can score or comment on *it* in turn). When you ask the GPT to score something, it signs both events atomically — the score and the rationale comment that justifies it — and cross-references them. Aggregators on this format treat unjustified scores as weight-zero by convention, so writing the rationale is part of writing the score, not optional.

**Private audiences are first-class.** v0.5 added named, end-to-end-encrypted groups: ask the GPT to *"create a 4A audience called team-design and mint an invite for Allison"* and it walks you through `createAudience` + `inviteToAudience`, returning a `4a://invite/...` URL plus a `https://claim.4a4.ai/...` twin you can paste into Slack. When Allison clicks the link and signs in, her claim event is queued; you then run `processAudienceClaims` to admit her (the gateway rotates the audience, drops the consumed invite, fans out fresh key-grants to every post-rotation member). From there, `publishToAudience` posts encrypted observations / claims / entities / relations / commons declarations into the group — NIP-44 v2 encrypted to the current epoch pubkey, NIP-17 gift-wrapped once per member so relays cannot map the membership graph. `audienceInbox` decrypts what's been published to you. **The full surface is 15 reserved kinds across two blocks and ~20 actions.**
...
**What's not.** Every public publish is **public and effectively permanent** — that's Nostr semantics. Deletion requests are advisory and not all relays honor them. The gateway stamps a `prov:wasAttributedTo` field on your public events carrying your OAuth login (`you@gmail.com` or your GitHub username) as a soft attribution. If you don't want your login on public events, use the [local CLI](https://github.com/evan108108/4a) with your own Nostr key instead.

**Audience caveats.** v0.5 audience writes are encrypted, not retractable. Anyone holding the audience's epoch private key — past or future members of that epoch — can decrypt every encrypted-variant rumor published while that epoch was current. Rotating to a new epoch does not retroactively re-encrypt history; it limits exposure of *future* writes. NIP-104 / MLS-on-Nostr will eventually offer forward and backward secrecy at the encryption layer; v0.5's wire shape is designed to be drop-in replaceable.
...
v0.5 audience starters (worth swapping into the pinned set once private-mode adoption picks up):

- *"Create a 4A audience called `team-design` and mint an invite URL for Allison."* — exercises `createAudience` + `inviteToAudience`. The GPT walks the user through the off-band invite flow before posting.
- *"Show me which 4A audiences I'm in, then read my inbox for `team-design`."* — exercises `listMyAudiences` + `audienceInbox`. Demonstrates capability-based decryption with no extra inputs.
- *"Publish an encrypted observation about `https://github.com/foo/bar` into `team-design`."* — exercises `publishToAudience` with `kind=30510`. One tool call NIP-44-encrypts the payload, builds the rumor, and gift-wraps it once per current member.
